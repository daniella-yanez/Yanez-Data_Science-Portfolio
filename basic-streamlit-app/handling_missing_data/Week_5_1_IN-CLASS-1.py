...
#column = st.selectbox("Choose a column to fill", df.select_dtypes(include=['number']).columns)
#letss the audience selcect a method with numeric values
# Apply the selected method to handle missing data.
st.dataframe(df[column])

#radio gives us five methods to delete or impute data
method= st.radio("Choose a method:",
         ["Original DF", "Drop Rows", "Drop Columns (>50% Missing)",
          "Impute Mean", "Impute Median", "Impute Zero"])

#copy original data frame
#df is going to remain untouched and df_clean is going to be our imputation / deletion data frame
df_clean = df.copy()
...
#online example: df[‘price’].fillna(df[‘price’].mean())
#prof final:df_clean[column] = df_clean[column].fillna(df[column].mean())
elif method == "Impute Mean":
    df_clean[column] = df_clean[column].fillna(df_clean[column].mean())
#online example:df[‘age’].fillna(df[‘age’].median())
#prof final: df_clean[column] = df_clean[column].fillna(df[column].median())
elif method == "Impute Median":
    df_clean[column] = df_clean[column].fillna(df_clean[column].median())
#online mode example: df[‘type_building’].fillna(df[‘type_building’].mode())
#prof final: df_clean[column] = df_clean[column].fillna(0)
